...
fn check_kitty_support() -> KittySupport {
    ...
    if has_remote_support(&mut stdout, &term).is_ok() {
...
fn has_remote_support(stdout: &mut impl Write, stdin: &impl ReadKey) -> ViuResult {
    // send the query
    write!(
        stdout,
...
mod tests {
    ...
    fn test_kitty_supported_remote_and_local() {
        ...
        has_remote_support(&mut stdout, &test_response).unwrap();
        ...
        has_local_support(&mut stdout, &test_response).unwrap();
        let result = std::str::from_utf8(&stdout).unwrap();
    ...
    fn test_no_kitty_support() {
        ...
        has_remote_support(&mut stdout, &test_response).unwrap_err();
        let result = std::str::from_utf8(&stdout).unwrap();
